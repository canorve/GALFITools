...
def main(): 
    ...
    parser = argparse.ArgumentParser(description="creates (or modify) a mask image for GALFIT from a ellipse region ds9 file ")


    parser.add_argument("ImageFile", help="the Mask image file to modify or create")
    parser.add_argument("RegFile", help="the region ds9 file which contains the ellipse regions to mask to modify or create")

    parser.add_argument("-n","--number", type=int, help="the value in counts to fill in the ellipse regions",default=0)
    ...
    for line in lines:
        ...
        #ellipse(3286,2398,812,1060,0)
        #ellipse(x,y,rx,ry,angle) #angle from Y-axis
    ...
    for idx, item in enumerate(xpos):

        #converting ellipse from DS9 ell to kron ellipse

        xx, yy, Rkron, theta, e = Ds9ell2Kronell(xpos[idx],ypos[idx],rx[idx],ry[idx],angle[idx])
