...
def main(): 
    ...
    lines = ReadFile(GalfitFile) #read  galfit input file

    (tmp)=GalfitFile.split(".")
    prefixname=tmp[0]


    for idx, item in enumerate(np.arange(1,number+1)): # cicle for every galfit file

        namefile = prefixname + "-" + str(item) + ".gal"

        fileout = open(namefile, "w")
        ...
        while index < len(lines):
            ...
            if tmp[0] == "4)":     # input image
                if param4:
                    c4min, c4max = param4[0],param4[1]
                    c4 = np.random.default_rng().uniform(c4min,c4max,number)
            ...
            if tmp[0] == "5)":     # input image
                if param5:
                    c5min, c5max = param5[0],param5[1]
                    c5 = np.random.default_rng().uniform(c5min,c5max,number)
            ...
            if tmp[0] == "6)":     # input image
                if param6:
                    c6min, c6max = param6[0],param6[1]
                    c6 = np.random.default_rng().uniform(c6min,c6max,number)
            ...
            if tmp[0] == "7)":     # input image
                if param7:
                    c7min, c7max = param7[0],param7[1]
                    c7 = np.random.default_rng().uniform(c7min,c7max,number)
            ...
            if tmp[0] == "8)":     # input image
                if param8:
                    c8min, c8max = param8[0],param8[1]
                    c8 = np.random.default_rng().uniform(c8min,c8max,number)
            ...
            if tmp[0] == "9)":     # input image
                if param9:
                    c9min, c9max = param9[0],param9[1]
                    c9 = np.random.default_rng().uniform(c9min,c9max,number)
            ...
            if tmp[0] == "10)":     # input image
                if param10:
                    c10min, c10max = param10[0],param10[1]
                    c10 = np.random.default_rng().uniform(c10min,c10max,number)
